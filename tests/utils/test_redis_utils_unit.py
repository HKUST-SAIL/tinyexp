...
from tinyexp.utils.redis_utils import RayRedisClusterManager, RedisClusterManager, RedisClusterStartupError
...
class _FakeRemoteActorClass:
    def __init__(self, hosts: list[str], actors: list[_FakeRayRedisActor], captured: dict[str, Any]) -> None:
        self._hosts = hosts
        self._actors = actors
        self._captured = captured
...
def _install_fake_ray(
    ...
    *,
    ...
    monkeypatch.setattr(
        "tinyexp.utils.redis_utils.ray.kill", lambda actor, no_restart: killed.append((actor, no_restart))
    )
    monkeypatch.setattr("tinyexp.utils.redis_utils.NodeAffinitySchedulingStrategy", _FakeNodeAffinitySchedulingStrategy)
...
def test_ray_redis_cluster_manager_starts_pinned_actors_and_creates_cluster(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_ray_redis_cluster_manager_standalone_starts_on_all_alive_nodes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "tinyexp.utils.redis_utils.subprocess.run", lambda *args, **kwargs: captured["commands"].append(args)
    )
