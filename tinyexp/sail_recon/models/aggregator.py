...
from tinyexp.sail_recon.layers import PatchEmbed
from tinyexp.sail_recon.layers.block import Block
from tinyexp.sail_recon.layers.rope import PositionGetter, RotaryPositionEmbedding2D
from tinyexp.sail_recon.layers.vision_transformer import (
    vit_base,
    vit_giant2,
    vit_large,
    vit_small,
...
class Aggregator(nn.Module):
    ...
    def __init__(
        ...
        self.register_token_reloc = nn.Parameter(torch.randn(1, 1, num_register_tokens, embed_dim))
            ...
            for attn_type in self.aa_order:
                if attn_type == "frame":
                    (
                        ...
                        frame_idx,
                        frame_intermediates,
                    ) = self._process_frame_attention(tokens, B, S, P, C, frame_idx, pos=pos)
                    ...
                    tokens = torch.ones((B, S, P, C), dtype=tokens.dtype, device=tokens.device)
                    tokens[:, no_reloc_list] = global_tokens.view(B, len(no_reloc_list), P, C)
            ...
            if frame_idx == self.aa_block_num:
                cam_token_last_layer = torch.cat(
                    [
                        ...
                        global_intermediates[-1].view(B, len(no_reloc_list), P, C)[:, :, 0],
                    ],
                    dim=-1,
                ).clone()  # [B, R, P, 2C]
        del concat_inter
    ...
    def forward_with_cache(
        ...
        camera_token = self.camera_token_reloc[:, 0:1].expand(B, S, *self.camera_token_reloc.shape[2:])
        register_token = self.register_token_reloc[:, 0:1].expand(B, S, *self.register_token_reloc.shape[2:])
        camera_token = camera_token.view(B * S, *camera_token.shape[2:])
        register_token = register_token.view(B * S, *register_token.shape[2:])
        ...
        attention_mask = attention_mask[
            ...
            -(self.rank + self.patch_start_idx) * self.num_recon - tokens.shape[0] * tokens.shape[1] :,
        ].to(tokens.device)
        attention_mask.unsqueeze_(0)  # (1, S, S)
        attention_mask.unsqueeze_(0)  # (1, 1, S, S)
        ...
        for _ in range(self.aa_block_num):
            ...
            for i in range(len(frame_intermediates)):
                ...
                if layer_idx + i in self.intermediate_layer_idx:
                    concat_inter = torch.cat([frame_intermediates[i], global_reloc_intermediates[i]], dim=-1)
                    output_dict[layer_idx + i] = concat_inter
    ...
    def random_select_features(self, feats, l_prime, device, pos=None):
        ...
        selected_features = torch.zeros(b, r, l_prime, c, device=device, dtype=feats.dtype)
        if pos is not None:
            b_p, r_p, l_p, c_p = pos.shape
            assert b_p == b and r_p == r and l_p == l, "pos must have the same shape as feats"
            # If pos is provided, ensure it has the same shape as feats
            selected_pos = torch.zeros(b_p, r_p, l_prime, c_p, device=device, dtype=pos.dtype)

        # Generate random indices for each batch sample using the rank-specific generator
        for i in range(b):
            for j in range(r):
                random_indices = torch.randperm(l, generator=self.generator)[:l_prime].to(device)
                selected_features[i][j] = feats[i, j, random_indices, :]
                selected_pos[i][j] = pos[i, j, random_indices, :] if pos is not None else None
    ...
    def _process_global_reloc_attention(
        ...
        if reloc_tokens.shape != (B, num_reloc * P, C):
            reloc_tokens = reloc_tokens.view(B, num_reloc, P, C).view(B, num_reloc * P, C)
        ...
        reloc_pos = torch.cat([downsampled_frame_pos, reloc_pos], dim=1) if downsampled_frame_pos is not None else None
        ...
        for _ in range(self.aa_block_size):
            if self.training:
                ...
                reloc_tokens = self.global_reloc_blocks[global_reloc_idx](reloc_tokens, reloc_pos, attention_mask)
            global_reloc_idx += 1
    ...
    def _process_global_reloc_attention_cache(self, tokens, B, S, P, C, global_reloc_idx, pos=None, attn_mask=None):
        """
        Process global attention blocks. We keep tokens in shape (B, S*P, C).
        """
        if tokens.shape != (B, S * P, C):
        ...
        for _ in range(self.aa_block_size):
            if self.training:
                ...
                tokens = self.global_reloc_blocks[global_reloc_idx](tokens, pos, attn_mask)
            global_reloc_idx += 1
