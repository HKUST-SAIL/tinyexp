...
import numpy as np
import torch
from eval.datasets.seven_scenes import SevenScenesUnifiedDataset
from eval.utils.device import to_cpu
from eval.utils.eval_utils import (
    align_gt_pred,
    save_all_intrinsics_to_txt,
    save_kitti_poses,
)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Configuration parameters for the reconstruction system")
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default="/horizon-bucket/saturn_v_4dlabel/008_Simulation/001_users/junyuan.deng/evaluation_datasets/dataset_reloc/sevenscenes_ms",
        help="Root directory of the dataset",
    )
    parser.add_argument(
        "--colmap_dir",
        type=str,
        default="/horizon-bucket/saturn_v_4dlabel/008_Simulation/001_users/junyuan.deng/evaluation_datasets/TankandTemple_ORI/colmap_results",
        help="Directory containing COLMAP results",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./output/reloc_7scenes",
    ...
    for scene_idx in range(len(scene_name_list)):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                dataset = SevenScenesUnifiedDataset(args.dataset_root, scene_name=scene_name_list[scene_idx])
                ...
                if processing_scene == "office":
                    train_samples[0], train_samples[8] = (
                        train_samples[8],
                        train_samples[0],
                    )
                elif processing_scene == "stairs":
                    train_samples[0], train_samples[43] = (
                        train_samples[43],
                        train_samples[0],
                    )
                else:
                ...
                poses_w2c_estimated = [one_result["extrinsic"][0].cpu().numpy() for one_result in result]
                poses_c2w_estimated = [
                    np.linalg.inv(np.vstack([pose, np.array([0, 0, 0, 1])])) for pose in poses_w2c_estimated
                ]
                aligned_poses, gt_poses = align_gt_pred(
                    all_data,
                    [poses_c2w_estimated],
                )
                gt_poses = [view["camera_pose"][0].cpu().numpy() for view in all_data]
                save_all_intrinsics_to_txt(result, filename=os.path.join(save_dir, "intrinsic_row.txt"))
                save_kitti_poses(
                    gt_poses,
                ...
                if args.save_dense_results:
                    depth_all = [one_result["depth_map"][0].cpu().numpy() for one_result in result]
                    point_map_unproj_all = [
                        one_result["point_map_by_unprojection"][0].cpu().numpy() for one_result in result
                    ]
                    point_map_all = [one_result["point_map"][0].cpu().numpy() for one_result in result]
                    np.savez_compressed(
                ...
                # evaluate the results using RANSAC-based alignment and report metrics
                from eval.utils.eval_pose_ransac import eval_pose_ransac

                eval_pose_ransac(
                    np.stack(gt_poses),
                    np.stack(aligned_poses),
                    save_dir=os.path.join(save_dir, "result.txt"),
                )
