...
import numpy as np
import torch
from eval.datasets.mip_360 import Mip360Dataset
...
def parse_args():
    ...
    parser.add_argument(
        "--dataset_root",
        type=str,
        default="/horizon-bucket/saturn_v_4dlabel/008_Simulation/001_users/junyuan.deng/evaluation_datasets/mip-nerf-360",
        help="Root directory of the dataset",
    )
    parser.add_argument(
        "--save_dir",
        type=str,
        default="./output/reloc_mip",
    ...
    for scene_idx in range(len(scene_name_list)):
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                dataset = Mip360Dataset(args.dataset_root, scene_name=scene_name_list[scene_idx])
