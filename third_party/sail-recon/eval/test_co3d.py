...
def setup_args():
    ...
    parser.add_argument(
        "--debug",
        action="store_true",
        help="Enable debug mode (only test on specific category)",
    )
    ...
    parser.add_argument(
        ...
        default="/horizon-bucket/saturn_v_4dlabel/009_geo/002_data/dust3r_datasets/dust3r_extracted/co3dv2",
    )
    parser.add_argument(
        "--co3d_anno_dir",
        type=str,
        help="Path to CO3D annotations",
        default="/horizon-bucket/saturn_v_4dlabel/008_Simulation/001_users/junyuan.deng/evaluation_datasets/co3d_v2_eval_vggt",
    )
    ...
    parser.add_argument("--recon", type=int, default=5, help="Number of reconstruction views")
    parser.add_argument("--reloc", type=int, default=5, help="Number of relocation views")
    parser.add_argument("--fixed_rank", type=int, default=300, help="Fixed rank for VGGT model")
...
def process_sequence(
    ...
    views = [{"img": images[i : i + 1]} for i in range(images.shape[0])]  # shape (1, 3, 518, 518)

    all_indices = list(range(10))
    random.shuffle(all_indices)

    print(f"Reconstruction views: {recon}, Relocation views: {reloc}, Fixed rank: {fixed_rank}")

    all_indices = list(range(recon + reloc))
    ...
    no_reloc_list += random.sample(remaining, recon - 1)

    # reloc_list 是剩下的
    # reloc_list = list(set(all_indices) - set(no_reloc_list))
    reloc_list = list(set(all_indices))
    ...
    if use_ba:
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(dtype=dtype):
                # extrinsic = model.tmp_forward2(views, no_reloc_list=no_reloc_list, reloc_list=reloc_list)
                torch.cuda.synchronize()
                start_event.record()
                extrinsic = model.co3d_forward(
                    views,
                ...
                end_event.record()
                torch.cuda.synchronize()
                t_process = start_event.elapsed_time(end_event)
            with torch.cuda.amp.autocast(dtype=dtype):
                pred_extrinsic = extrinsic[0]
    ...
    with torch.cuda.amp.autocast(dtype=torch.float64):
        ...
        print(f"{category} sequence {seq_name} time: {t_process:.2f} ms")

        return rel_rangle_deg.cpu().numpy(), rel_tangle_deg.cpu().numpy(), t_process
...
def main():
    ...
    for category in SEEN_CATEGORIES:
        ...
        if args.fast_eval and len(seq_names) >= 10:
            seq_names = random.sample(seq_names, 2)
        ...
        for seq_name in seq_names:
            ...
            seq_rError, seq_tError, t_process = process_sequence(
                model,
                seq_name,
                seq_data,
                category,
        ...
        Racc_15 = (rError < 15).mean().item()
        Tacc_15 = (tError < 15).mean().item()
        ...
        print(
            f"{BOLD}{BLUE}AUC of {category} test set:{RESET} {GREEN}{Auc_30:.4f} (AUC@30), {Auc_15:.4f} (AUC@15), {Auc_5:.4f} (AUC@5), {Auc_3:.4f} (AUC@3){RESET}"
        )
        ...
        Racc_15_by_now = np.mean([per_category_results[category]["Racc_15"] for category in per_category_results])
        Tacc_15_by_now = np.mean([per_category_results[category]["Tacc_15"] for category in per_category_results])
        t_process_by_now = np.mean([per_category_results[category]["time"] for category in per_category_results])
        print(
            f"{BOLD}{BLUE}Mean AUC of categories by now:{RESET} {RED}{mean_AUC_30_by_now:.4f} (AUC@30), {mean_AUC_15_by_now:.4f} (AUC@15), {mean_AUC_5_by_now:.4f} (AUC@5), {mean_AUC_3_by_now:.4f} (AUC@3){RESET}"
        )
        # print(f"{BOLD}{BLUE}Racc_15 of {category} test set:{RESET} {GREEN}{Racc_15:.4f}, Tacc_15: {Tacc_15:.4f}{RESET}")
        print(
            f"{BOLD}{BLUE}Mean Racc_15 of categories by now:{RESET} {RED}{Racc_15_by_now:.4f}, Tacc_15: {Tacc_15_by_now:.4f}{RESET}"
        )
        print(f"{BOLD}{BLUE}Average time of processing each sequence:{RESET} {RED}{t_process_by_now:.2f} ms{RESET}")
        print("=" * 80)
    ...
    for category in sorted(per_category_results.keys()):
        print(
            f"{category:<15}: {per_category_results[category]['Auc_30']:.4f} (AUC@30), {per_category_results[category]['Auc_15']:.4f} (AUC@15), {per_category_results[category]['Auc_5']:.4f} (AUC@5), {per_category_results[category]['Auc_3']:.4f} (AUC@3), {per_category_results[category]['Racc_15']:.4f}, {per_category_results[category]['Tacc_15']:.4f}"
        )
    ...
    if per_category_results:
        ...
        mean_Racc_15 = np.mean([per_category_results[category]["Racc_15"] for category in per_category_results])
        mean_Tacc_15 = np.mean([per_category_results[category]["Tacc_15"] for category in per_category_results])
        mean_time = np.mean([per_category_results[category]["time"] for category in per_category_results])
        print("-" * 50)
        print(
            f"Mean AUC: {mean_AUC_30:.4f} (AUC@30), {mean_AUC_15:.4f} (AUC@15), {mean_AUC_5:.4f} (AUC@5), {mean_AUC_3:.4f} (AUC@3), Racc_15: {mean_Racc_15:.4f}, Tacc_15: {mean_Tacc_15:.4f}, Time: {mean_time:.2f} ms"
        )
