...
def save_pointcloud_with_plyfile(result, filename="output.ply", downsample_ratio=10):
    ...
    for view in result:
        ...
        N = pts.shape[0]
        if downsample_ratio > 1 and downsample_ratio <= N:
...
def save_pointcloud_with_plyfile_each_frame(result, filename="output.ply", downsample_ratio=10):
    for frame_number, view in enumerate(result):
        ...
        # Save with plyfile
        el = PlyElement.describe(vertex_data, "vertex")
        PlyData([el], text=False).write(filename.split(".ply")[0] + f"_{frame_number:05d}.ply")

        print(f"[PLY] Saved {len(all_points)} points to {filename} idx {frame_number:05d}")
