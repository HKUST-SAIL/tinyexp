...
class SailRecon(nn.Module, PyTorchModelHubMixin):
    ...
    def tmp_forward(self, views, no_reloc_list=None, reloc_list=[], fix_rank=300):
        ...
        rgb_feats, idx_patch, cam_token_last_layer = self.aggregator(rgbs, no_reloc_list, reloc_list, fix_rank=fix_rank)
    ...
    def reloc(
        ...
        rgb_feats, idx_patch = self.aggregator.forward_with_cache(rgbs, fix_rank=fix_rank)
        ...
        with torch.cuda.amp.autocast(enabled=False):
            # Camera decoder head
            cam_maps = self.camera_head(rgb_feats, self.cam_token_last_layer)  # [(B, S, 9), ...]
            extrinsic, intrinsic = pose_encoding_to_extri_intri(cam_maps[-1], (rgbs.shape[-2], rgbs.shape[-1]))
            ...
            xyz_map, xyz_cnf = self.point_head(rgb_feats, images=rgbs, patch_start_idx=idx_patch)

            dpt_map, dpt_cnf = self.depth_head(rgb_feats, images=rgbs, patch_start_idx=idx_patch)
            ...
            for i in range(len(views)):
                ...
                for count_value in torch.arange(1, 5.5, 0.25):
                    conf_percentage_count_perview.append((xyz_cnf[:, i] > count_value).sum(dim=(1, 2)).item() / Hn / Wn)
    ...
    def co3d_forward(self, views, no_reloc_list=None, reloc_list=None, fix_rank=None):
        rgbs_no_reloc = torch.cat([view["img"] for view in [views[i] for i in no_reloc_list]], dim=0)
        rgb_reloc = torch.cat([view["img"] for view in [views[i] for i in reloc_list]], dim=0)
        ...
        with torch.cuda.amp.autocast(dtype=torch.float32):
            cam_maps = self.camera_head(rgb_feats, cam_token_last_layer)  # [(B, S, 9), ...]
